...
def draw_full_detail_label(c, data, x, y, width, height, seller_info):
    ...
    # --- [추가] 이력번호 그리기 ---
    history_num = data.get('history', '').strip()
    details_start_y = y + height - 65 # 상세정보 텍스트 기본 시작 Y좌표

    if history_num:
        # 가공일(y - 45)보다 10만큼 아래(y - 55)에 이력번호 출력
        c.drawString(x + 10, y + height - 55, f"[이력번호: {history_num}]")
        # 이력번호가 있으면 공간 확보를 위해 상세정보 텍스트를 약간 더 아래로(-70) 내림
        details_start_y = y + height - 70
    # ------------------------------
    ...
    text_object = c.beginText(x + 10, details_start_y)
